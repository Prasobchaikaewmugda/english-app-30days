lessons_data = {

    # ============================================================
    # DAY 1: การทักทายหน้าเคาน์เตอร์
    # ============================================================
    "Day 1": {
        "title": "👋 Day 1: การทักทายหน้าเคาน์เตอร์",
        "phrases": [
            {
                "english":       "Good morning! Welcome to the hospital.",
                "pronunciation": "กุด มอร์-นิ่ง! เวล-คัม ทู เดอะ ฮอส-พิ-ทัล.",
                "meaning":       "สวัสดีตอนเช้า! ยินดีต้อนรับสู่โรงพยาบาล"
            },
            {
                "english":       "Good afternoon! How can I help you today?",
                "pronunciation": "กุด อาฟ-เตอร์-นูน! ฮาว แคน ไอ เฮลพ์ ยู ทู-เดย์?",
                "meaning":       "สวัสดีตอนบ่าย! วันนี้ให้ช่วยอะไรได้บ้างคะ/ครับ?"
            },
            {
                "english":       "Please take a number and have a seat.",
                "pronunciation": "พลีส เทค อะ นัม-เบอร์ แอนด์ แฮฟ อะ ซีท.",
                "meaning":       "กรุณารับบัตรคิวและนั่งรอได้เลยค่ะ/ครับ"
            },
            {
                "english":       "May I have your hospital card, please?",
                "pronunciation": "เมย์ ไอ แฮฟ ยัวร์ ฮอส-พิ-ทัล คาร์ด, พลีส?",
                "meaning":       "ขอบัตรโรงพยาบาลของท่านด้วยได้ไหมคะ/ครับ?"
            },
            {
                "english":       "Do you have an appointment today?",
                "pronunciation": "ดู ยู แฮฟ แอน อะ-พอยน์-เมนท์ ทู-เดย์?",
                "meaning":       "วันนี้ท่านมีนัดไว้ก่อนไหมคะ/ครับ?"
            },
            {
                "english":       "Which department would you like to visit?",
                "pronunciation": "วิช ดี-พาร์ท-เมนท์ วูด ยู ไลค์ ทู วิซ-ซิท?",
                "meaning":       "ท่านต้องการไปแผนกไหนคะ/ครับ?"
            },
            {
                "english":       "Please fill in this form first.",
                "pronunciation": "พลีส ฟิล อิน ดิส ฟอร์ม เฟิร์สท์.",
                "meaning":       "กรุณากรอกแบบฟอร์มนี้ก่อนนะคะ/ครับ"
            },
            {
                "english":       "The doctor will see you in about twenty minutes.",
                "pronunciation": "เดอะ ด็อก-เตอร์ วิล ซี ยู อิน อะ-เบาท์ ทเวน-ที่ มิน-นิทส์.",
                "meaning":       "หมอจะพบท่านในอีกประมาณยี่สิบนาทีนะคะ/ครับ"
            },
            {
                "english":       "Is this your first visit to our hospital?",
                "pronunciation": "อีส ดิส ยัวร์ เฟิร์สท์ วิซ-ซิท ทู อาวร์ ฮอส-พิ-ทัล?",
                "meaning":       "นี่เป็นครั้งแรกที่ท่านมาโรงพยาบาลของเราไหมคะ/ครับ?"
            },
            {
                "english":       "Have a good day! Please come again.",
                "pronunciation": "แฮฟ อะ กุด เดย์! พลีส คัม อะ-เกน.",
                "meaning":       "ขอให้มีวันที่ดีนะคะ/ครับ! ยินดีต้อนรับเสมอ"
            },
        ],
        "quizzes": [
            {
                "question": "'May I have your hospital card, please?' แปลว่าอะไร?",
                "choices":  [
                    "ท่านมีนัดไว้ก่อนไหม?",
                    "ขอบัตรโรงพยาบาลของท่านด้วยได้ไหมคะ/ครับ?",
                    "กรุณากรอกแบบฟอร์มนี้ก่อนนะคะ/ครับ",
                ],
                "answer":   "ขอบัตรโรงพยาบาลของท่านด้วยได้ไหมคะ/ครับ?"
            },
            {
                "question": "ถ้าอยากถามว่า 'ท่านต้องการไปแผนกไหน?' ควรพูดว่าอะไร?",
                "choices":  [
                    "Do you have an appointment today?",
                    "Which department would you like to visit?",
                    "Is this your first visit to our hospital?",
                ],
                "answer":   "Which department would you like to visit?"
            },
            {
                "question": "'Please take a number and have a seat.' แปลว่าอะไร?",
                "choices":  [
                    "กรุณารับบัตรคิวและนั่งรอได้เลยค่ะ/ครับ",
                    "หมอจะพบท่านในอีกยี่สิบนาที",
                    "ขอให้มีวันที่ดีนะคะ/ครับ",
                ],
                "answer":   "กรุณารับบัตรคิวและนั่งรอได้เลยค่ะ/ครับ"
            },
        ]
    },

    # ============================================================
    # DAY 2: การกล่าวคำขอบคุณและตอบรับ
    # ============================================================
    "Day 2": {
        "title": "🙏 Day 2: การกล่าวคำขอบคุณและตอบรับ",
        "phrases": [
            {
                "english":       "Thank you so much for your help.",
                "pronunciation": "แธงค์ ยู โซ มัช ฟอร์ ยัวร์ เฮลพ์.",
                "meaning":       "ขอบคุณมากสำหรับความช่วยเหลือของคุณ"
            },
            {
                "english":       "You're welcome! It's my pleasure.",
                "pronunciation": "ยัวร์ เวล-คัม! อิทส์ มาย เพลช-เชอร์.",
                "meaning":       "ยินดีเลยค่ะ/ครับ! เป็นความยินดีของฉัน"
            },
            {
                "english":       "I really appreciate your patience.",
                "pronunciation": "ไอ เรียล-ลี่ แอพ-พรี-ชี-เอท ยัวร์ เพ-เชินซ์.",
                "meaning":       "ขอบคุณที่รอด้วยความอดทนนะคะ/ครับ"
            },
            {
                "english":       "No problem at all! Glad I could help.",
                "pronunciation": "โน พ็อบ-เล็ม แอท ออล! แกลด ไอ คุด เฮลพ์.",
                "meaning":       "ไม่มีปัญหาเลยค่ะ/ครับ! ดีใจที่ได้ช่วย"
            },
            {
                "english":       "That's very kind of you to say so.",
                "pronunciation": "แดทส์ เวรี่ ไคนด์ ออฟ ยู ทู เซย์ โซ.",
                "meaning":       "คุณพูดดีมากเลยนะคะ/ครับ"
            },
            {
                "english":       "Thank you for waiting so patiently.",
                "pronunciation": "แธงค์ ยู ฟอร์ เวท-ติ้ง โซ เพ-เชินท์-ลี่.",
                "meaning":       "ขอบคุณที่รอด้วยความอดทนนะคะ/ครับ"
            },
            {
                "english":       "I can't thank you enough for everything.",
                "pronunciation": "ไอ แคนท์ แธงค์ ยู อิ-นัฟ ฟอร์ เอฟ-รี-ธิ่ง.",
                "meaning":       "ขอบคุณสำหรับทุกอย่างจริงๆ ค่ะ/ครับ"
            },
            {
                "english":       "Don't mention it. It's part of my job.",
                "pronunciation": "โดนท์ เมน-ชัน อิท. อิทส์ พาร์ท ออฟ มาย จ็อบ.",
                "meaning":       "ไม่ต้องพูดถึงเลยค่ะ/ครับ นี่คืองานของฉัน"
            },
            {
                "english":       "We appreciate you choosing our hospital.",
                "pronunciation": "วี แอพ-พรี-ชี-เอท ยู ชูส-ซิ่ง อาวร์ ฮอส-พิ-ทัล.",
                "meaning":       "เราขอบคุณที่ท่านเลือกโรงพยาบาลของเรา"
            },
            {
                "english":       "You are always welcome here anytime.",
                "pronunciation": "ยู อาร์ ออล-เวย์ส เวล-คัม เฮียร์ เอ็น-นี่-ไทม์.",
                "meaning":       "ท่านยินดีต้อนรับที่นี่ได้ตลอดเวลาเลยนะคะ/ครับ"
            },
        ],
        "quizzes": [
            {
                "question": "'No problem at all! Glad I could help.' แปลว่าอะไร?",
                "choices":  [
                    "ขอโทษที่ทำให้รอนานนะคะ/ครับ",
                    "ไม่มีปัญหาเลยค่ะ/ครับ! ดีใจที่ได้ช่วย",
                    "ขอบคุณสำหรับทุกอย่างค่ะ/ครับ",
                ],
                "answer":   "ไม่มีปัญหาเลยค่ะ/ครับ! ดีใจที่ได้ช่วย"
            },
            {
                "question": "ถ้าอยากพูดว่า 'ยินดีเลยค่ะ เป็นความยินดีของฉัน' ควรพูดว่าอะไร?",
                "choices":  [
                    "Thank you so much for your help.",
                    "You're welcome! It's my pleasure.",
                    "That's very kind of you to say so.",
                ],
                "answer":   "You're welcome! It's my pleasure."
            },
            {
                "question": "'Don't mention it. It's part of my job.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันไม่ได้ยินสิ่งที่คุณพูด",
                    "นี่ไม่ใช่หน้าที่ของฉัน",
                    "ไม่ต้องพูดถึงเลยค่ะ/ครับ นี่คืองานของฉัน",
                ],
                "answer":   "ไม่ต้องพูดถึงเลยค่ะ/ครับ นี่คืองานของฉัน"
            },
        ]
    },

    # ============================================================
    # DAY 3: การจ่ายยาและวิธีใช้ยา
    # ============================================================
    "Day 3": {
        "title": "💊 Day 3: การจ่ายยาและวิธีใช้ยา",
        "phrases": [
            {
                "english":       "Take one tablet three times a day after meals.",
                "pronunciation": "เทค วัน แท็บ-เล็ท ธรี ไทม์ส อะ เดย์ อาฟ-เตอร์ มีลส์.",
                "meaning":       "รับประทานยาครั้งละ 1 เม็ด วันละ 3 ครั้ง หลังอาหาร"
            },
            {
                "english":       "Do you have any known drug allergies?",
                "pronunciation": "ดู ยู แฮฟ เอ็นนี่ โนน ดรัก แอล-เลอร์-จีส์?",
                "meaning":       "ท่านมีประวัติแพ้ยาชนิดใดบ้างไหมคะ/ครับ?"
            },
            {
                "english":       "This medicine may cause drowsiness. Do not drive.",
                "pronunciation": "ดิส เมด-ดิ-ซิน เมย์ คอส ดรา-ซี-เนส. ดู น็อท ไดรฟ์.",
                "meaning":       "ยานี้อาจทำให้ง่วงนอน ไม่ควรขับรถนะคะ/ครับ"
            },
            {
                "english":       "Please take this antibiotic for seven days.",
                "pronunciation": "พลีส เทค ดิส แอน-ที่-ไบ-ออ-ทิค ฟอร์ เซฟ-เวน เดย์ส.",
                "meaning":       "กรุณารับประทานยาปฏิชีวนะนี้ให้ครบเจ็ดวันนะคะ/ครับ"
            },
            {
                "english":       "Apply this cream to the affected area twice daily.",
                "pronunciation": "แอพ-พลาย ดิส ครีม ทู ดิ แอฟ-เฟค-เทด แอ-เรีย ทไวซ์ เดย์-ลี่.",
                "meaning":       "ทาครีมนี้บริเวณที่มีอาการวันละสองครั้งนะคะ/ครับ"
            },
            {
                "english":       "Take this medicine on an empty stomach.",
                "pronunciation": "เทค ดิส เมด-ดิ-ซิน ออน แอน เอ็ม-ที่ สตัม-มัค.",
                "meaning":       "รับประทานยานี้ตอนท้องว่างนะคะ/ครับ"
            },
            {
                "english":       "Do not exceed the recommended dosage.",
                "pronunciation": "ดู น็อท อิค-ซีด เดอะ เร็ค-คัม-เมน-เดด โด-สิจ.",
                "meaning":       "ไม่ควรรับประทานยาเกินขนาดที่แนะนำนะคะ/ครับ"
            },
            {
                "english":       "Store this medicine in a cool, dry place.",
                "pronunciation": "สตอร์ ดิส เมด-ดิ-ซิน อิน อะ คูล, ดราย เพลส.",
                "meaning":       "เก็บยานี้ไว้ในที่เย็นและแห้งนะคะ/ครับ"
            },
            {
                "english":       "Please come back if your symptoms get worse.",
                "pronunciation": "พลีส คัม แบ็ค อิฟ ยัวร์ ซิมป์-ทัมส์ เก็ท เวิร์ส.",
                "meaning":       "ถ้าอาการแย่ลง กรุณากลับมาพบแพทย์อีกครั้งนะคะ/ครับ"
            },
            {
                "english":       "Here are your medicines. You have three types today.",
                "pronunciation": "เฮียร์ อาร์ ยัวร์ เมด-ดิ-ซินส์. ยู แฮฟ ธรี ไทพส์ ทู-เดย์.",
                "meaning":       "นี่คือยาของท่านนะคะ/ครับ วันนี้มียาสามชนิด"
            },
        ],
        "quizzes": [
            {
                "question": "'Take one tablet three times a day after meals.' แปลว่าอะไร?",
                "choices":  [
                    "รับประทานยาครั้งละ 3 เม็ด วันละครั้ง ก่อนอาหาร",
                    "รับประทานยาครั้งละ 1 เม็ด วันละ 3 ครั้ง หลังอาหาร",
                    "รับประทานยาครั้งละ 1 เม็ด วันละ 2 ครั้ง หลังอาหาร",
                ],
                "answer":   "รับประทานยาครั้งละ 1 เม็ด วันละ 3 ครั้ง หลังอาหาร"
            },
            {
                "question": "ถ้าต้องการบอกว่า 'ยานี้อาจทำให้ง่วงนอน' ควรพูดว่าอะไร?",
                "choices":  [
                    "Do not exceed the recommended dosage.",
                    "Store this medicine in a cool, dry place.",
                    "This medicine may cause drowsiness.",
                ],
                "answer":   "This medicine may cause drowsiness."
            },
            {
                "question": "'Please come back if your symptoms get worse.' แปลว่าอะไร?",
                "choices":  [
                    "กรุณารับประทานยาให้ครบนะคะ/ครับ",
                    "ถ้าอาการแย่ลง กรุณากลับมาพบแพทย์อีกครั้งนะคะ/ครับ",
                    "ไม่ควรรับประทานยาเกินขนาดนะคะ/ครับ",
                ],
                "answer":   "ถ้าอาการแย่ลง กรุณากลับมาพบแพทย์อีกครั้งนะคะ/ครับ"
            },
        ]
    },

    # ============================================================
    # DAY 4: การแนะนำตนเองและผู้อื่น
    # ============================================================
    "Day 4": {
        "title": "🙋 Day 4: การแนะนำตนเองและผู้อื่น",
        "phrases": [
            {
                "english":       "Hello! My name is Ploy. I'm a nurse here.",
                "pronunciation": "เฮล-โล! มาย เนม อีส พลอย. ไอม์ อะ เนิร์ส เฮียร์.",
                "meaning":       "สวัสดีคะ/ครับ! ฉันชื่อพลอย เป็นพยาบาลที่นี่"
            },
            {
                "english":       "Nice to meet you! Where are you from?",
                "pronunciation": "ไนซ์ ทู มีท ยู! แวร์ อาร์ ยู ฟรอม?",
                "meaning":       "ยินดีที่ได้รู้จักนะคะ/ครับ! คุณมาจากที่ไหน?"
            },
            {
                "english":       "I'd like to introduce my supervisor, Dr. Somsak.",
                "pronunciation": "ไอด์ ไลค์ ทู อิน-โทร-ดิวส์ มาย ซู-เพอร์-วาย-เซอร์, ด็อก-เตอร์ สมศักดิ์.",
                "meaning":       "ขอแนะนำผู้บังคับบัญชาของฉัน คุณหมอสมศักดิ์นะคะ/ครับ"
            },
            {
                "english":       "She is the head pharmacist of our department.",
                "pronunciation": "ชี อีส เดอะ เฮด ฟาร์-มะ-ซิสท์ ออฟ อาวร์ ดี-พาร์ท-เมนท์.",
                "meaning":       "เธอเป็นหัวหน้าเภสัชกรของแผนกเรา"
            },
            {
                "english":       "I have been working here for three years.",
                "pronunciation": "ไอ แฮฟ บีน เวิร์ค-กิ้ง เฮียร์ ฟอร์ ธรี เยียร์ส.",
                "meaning":       "ฉันทำงานที่นี่มาสามปีแล้วค่ะ/ครับ"
            },
            {
                "english":       "What is your name, and how do you spell it?",
                "pronunciation": "วอท อีส ยัวร์ เนม, แอนด์ ฮาว ดู ยู สเพล อิท?",
                "meaning":       "ชื่ออะไรคะ/ครับ และสะกดอย่างไร?"
            },
            {
                "english":       "I work in the outpatient department.",
                "pronunciation": "ไอ เวิร์ค อิน ดิ เอาท์-เพ-เชินท์ ดี-พาร์ท-เมนท์.",
                "meaning":       "ฉันทำงานอยู่แผนกผู้ป่วยนอกค่ะ/ครับ"
            },
            {
                "english":       "This is my colleague, Khun Mint.",
                "pronunciation": "ดิส อีส มาย คอล-ลีก, คุณ มิ้นท์.",
                "meaning":       "นี่คือเพื่อนร่วมงานของฉัน คุณมิ้นท์ค่ะ/ครับ"
            },
            {
                "english":       "Feel free to ask me if you have any questions.",
                "pronunciation": "ฟีล ฟรี ทู แอสค์ มี อิฟ ยู แฮฟ เอ็นนี่ เควส-ชันส์.",
                "meaning":       "มีคำถามอะไรถามฉันได้เลยนะคะ/ครับ"
            },
            {
                "english":       "It's a pleasure to have you on our team.",
                "pronunciation": "อิทส์ อะ เพลช-เชอร์ ทู แฮฟ ยู ออน อาวร์ ทีม.",
                "meaning":       "ยินดีที่มีคุณเป็นส่วนหนึ่งของทีมเรานะคะ/ครับ"
            },
        ],
        "quizzes": [
            {
                "question": "'I'd like to introduce my supervisor.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันอยากรู้จักผู้บังคับบัญชาของคุณ",
                    "ขอแนะนำผู้บังคับบัญชาของฉันนะคะ/ครับ",
                    "ผู้บังคับบัญชาของฉันชื่ออะไร?",
                ],
                "answer":   "ขอแนะนำผู้บังคับบัญชาของฉันนะคะ/ครับ"
            },
            {
                "question": "ถ้าต้องการบอกว่า 'ฉันทำงานอยู่แผนกผู้ป่วยนอก' ควรพูดว่าอะไร?",
                "choices":  [
                    "I work in the emergency department.",
                    "I work in the outpatient department.",
                    "I work in the inpatient department.",
                ],
                "answer":   "I work in the outpatient department."
            },
            {
                "question": "'Feel free to ask me if you have any questions.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันไม่สามารถตอบคำถามได้",
                    "กรุณาส่งคำถามมาทางอีเมล",
                    "มีคำถามอะไรถามฉันได้เลยนะคะ/ครับ",
                ],
                "answer":   "มีคำถามอะไรถามฉันได้เลยนะคะ/ครับ"
            },
        ]
    },

    # ============================================================
    # DAY 5: นิสัยและกิจวัตรประจำวัน
    # ============================================================
    "Day 5": {
        "title": "⏰ Day 5: นิสัยและกิจวัตรประจำวัน",
        "phrases": [
            {
                "english":       "I usually wake up at six o'clock every morning.",
                "pronunciation": "ไอ ยู-ชู-อัล-ลี่ เวค อัพ แอท ซิกซ์ โอ-คล็อค เอฟ-รี่ มอร์-นิ่ง.",
                "meaning":       "ฉันมักตื่นนอนตอนหกโมงเช้าทุกวัน"
            },
            {
                "english":       "I always eat breakfast before I go to work.",
                "pronunciation": "ไอ ออล-เวย์ส อีท เบรค-เฟิสท์ บีฟอร์ ไอ โก ทู เวิร์ค.",
                "meaning":       "ฉันกินข้าวเช้าก่อนไปทำงานเสมอ"
            },
            {
                "english":       "My shift starts at eight and ends at four.",
                "pronunciation": "มาย ชิฟท์ สตาร์ทส์ แอท เอท แอนด์ เอนด์ส แอท ฟอร์.",
                "meaning":       "กะงานของฉันเริ่มแปดโมงเช้าและเลิกสี่โมงเย็น"
            },
            {
                "english":       "I usually have lunch with my colleagues.",
                "pronunciation": "ไอ ยู-ชู-อัล-ลี่ แฮฟ ลันช์ วิธ มาย คอล-ลีกส์.",
                "meaning":       "ฉันมักกินข้าวกลางวันกับเพื่อนร่วมงาน"
            },
            {
                "english":       "I exercise for thirty minutes before bed.",
                "pronunciation": "ไอ เอ็กซ์-เออร์-ไซซ์ ฟอร์ เธอร์-ที่ มิน-นิทส์ บีฟอร์ เบด.",
                "meaning":       "ฉันออกกำลังกายสามสิบนาทีก่อนนอน"
            },
            {
                "english":       "I sometimes work overtime on weekdays.",
                "pronunciation": "ไอ ซัม-ไทม์ส เวิร์ค โอ-เวอร์-ไทม์ ออน วีค-เดย์ส.",
                "meaning":       "บางครั้งฉันทำงานล่วงเวลาในวันธรรมดา"
            },
            {
                "english":       "I never skip my morning coffee.",
                "pronunciation": "ไอ เนฟ-เวอร์ สกิพ มาย มอร์-นิ่ง คอฟ-ฟี่.",
                "meaning":       "ฉันไม่เคยขาดกาแฟตอนเช้าเลย"
            },
            {
                "english":       "On weekends I like to sleep in a little.",
                "pronunciation": "ออน วีค-เอนด์ส ไอ ไลค์ ทู สลีพ อิน อะ ลิต-เทิล.",
                "meaning":       "วันหยุดสุดสัปดาห์ฉันชอบนอนตื่นสายนิดหน่อย"
            },
            {
                "english":       "I try to drink eight glasses of water every day.",
                "pronunciation": "ไอ ทราย ทู ดริงค์ เอท กลาส-เซส ออฟ วอ-เตอร์ เอฟ-รี่ เดย์.",
                "meaning":       "ฉันพยายามดื่มน้ำแปดแก้วทุกวัน"
            },
            {
                "english":       "I go to bed at around ten o'clock at night.",
                "pronunciation": "ไอ โก ทู เบด แอท อะ-เรานด์ เท็น โอ-คล็อค แอท ไนท์.",
                "meaning":       "ฉันเข้านอนประมาณสี่ทุ่มในแต่ละคืน"
            },
        ],
        "quizzes": [
            {
                "question": "'My shift starts at eight and ends at four.' แปลว่าอะไร?",
                "choices":  [
                    "กะงานของฉันเริ่มแปดโมงเช้าและเลิกสี่โมงเย็น",
                    "ฉันทำงานแปดชั่วโมงต่อวัน",
                    "ฉันเริ่มงานตอนบ่ายสี่โมง",
                ],
                "answer":   "กะงานของฉันเริ่มแปดโมงเช้าและเลิกสี่โมงเย็น"
            },
            {
                "question": "ถ้าอยากบอกว่า 'ฉันชอบนอนตื่นสายในวันหยุด' ควรพูดว่าอะไร?",
                "choices":  [
                    "I always wake up early on weekends.",
                    "On weekends I like to sleep in a little.",
                    "I never rest on weekends.",
                ],
                "answer":   "On weekends I like to sleep in a little."
            },
            {
                "question": "'I try to drink eight glasses of water every day.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันดื่มน้ำแปดแก้วทุกวันโดยไม่ขาด",
                    "ฉันพยายามดื่มน้ำแปดแก้วทุกวัน",
                    "หมอแนะนำให้ฉันดื่มน้ำแปดแก้วต่อวัน",
                ],
                "answer":   "ฉันพยายามดื่มน้ำแปดแก้วทุกวัน"
            },
        ]
    },

    # ============================================================
    # DAY 6: การกล่าวคำขอโทษและการอำลา
    # ============================================================
    "Day 6": {
        "title": "😊 Day 6: การกล่าวคำขอโทษและการอำลา",
        "phrases": [
            {
                "english":       "I'm so sorry to keep you waiting.",
                "pronunciation": "ไอม์ โซ ซอ-รี่ ทู คีพ ยู เวท-ติ้ง.",
                "meaning":       "ขอโทษมากที่ทำให้รอนานนะคะ/ครับ"
            },
            {
                "english":       "Please accept my sincerest apologies.",
                "pronunciation": "พลีส แอค-เซ็พท์ มาย ซิน-เซียร์-เอสท์ อะ-พ็อล-โล-จีส์.",
                "meaning":       "กรุณายอมรับคำขอโทษอย่างจริงใจของฉันด้วยนะคะ/ครับ"
            },
            {
                "english":       "I'm terribly sorry. That was my mistake.",
                "pronunciation": "ไอม์ เทอ-ริบ-ลี่ ซอ-รี่. แดท วอส มาย มิส-เทค.",
                "meaning":       "ขอโทษอย่างมากเลยค่ะ/ครับ นั่นเป็นความผิดพลาดของฉัน"
            },
            {
                "english":       "It's completely fine. Please don't worry about it.",
                "pronunciation": "อิทส์ คอม-พลีท-ลี่ ไฟน์. พลีส โดนท์ วัร์-รี่ อะ-เบาท์ อิท.",
                "meaning":       "ไม่เป็นไรเลยค่ะ/ครับ ไม่ต้องกังวลนะ"
            },
            {
                "english":       "I forgive you. These things happen.",
                "pronunciation": "ไอ ฟอร์-กิฟ ยู. ดีส ธิงส์ แฮพ-เพน.",
                "meaning":       "ฉันให้อภัยคุณแล้ว สิ่งเหล่านี้มันเกิดขึ้นได้"
            },
            {
                "english":       "Goodbye! Take care and stay healthy.",
                "pronunciation": "กุด-บาย! เทค แคร์ แอนด์ สเตย์ เฮล-ธี่.",
                "meaning":       "ลาก่อนนะคะ/ครับ! ดูแลสุขภาพด้วย"
            },
            {
                "english":       "See you at your next appointment.",
                "pronunciation": "ซี ยู แอท ยัวร์ เน็กซ์ อะ-พอยน์-เมนท์.",
                "meaning":       "เจอกันตอนนัดครั้งต่อไปนะคะ/ครับ"
            },
            {
                "english":       "It was wonderful working with you today.",
                "pronunciation": "อิท วอส วอน-เดอร์-ฟูล เวิร์ค-กิ้ง วิธ ยู ทู-เดย์.",
                "meaning":       "วันนี้ยินดีมากที่ได้ทำงานด้วยกันนะคะ/ครับ"
            },
            {
                "english":       "Drive safely and get home soon.",
                "pronunciation": "ไดรฟ์ เซฟ-ลี่ แอนด์ เก็ท โฮม ซูน.",
                "meaning":       "ขับรถระวังด้วยนะคะ/ครับ ถึงบ้านปลอดภัย"
            },
            {
                "english":       "Thank you for coming. We hope to see you again.",
                "pronunciation": "แธงค์ ยู ฟอร์ คัม-มิ่ง. วี โฮพ ทู ซี ยู อะ-เกน.",
                "meaning":       "ขอบคุณที่มานะคะ/ครับ หวังว่าจะได้พบกันอีก"
            },
        ],
        "quizzes": [
            {
                "question": "'I'm so sorry to keep you waiting.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันรอคุณมานานมากแล้ว",
                    "ขอโทษมากที่ทำให้รอนานนะคะ/ครับ",
                    "กรุณารอสักครู่นะคะ/ครับ",
                ],
                "answer":   "ขอโทษมากที่ทำให้รอนานนะคะ/ครับ"
            },
            {
                "question": "ถ้าอยากพูดว่า 'ลาก่อน ดูแลสุขภาพด้วย' ควรพูดว่าอะไร?",
                "choices":  [
                    "See you at your next appointment.",
                    "Thank you for coming. We hope to see you again.",
                    "Goodbye! Take care and stay healthy.",
                ],
                "answer":   "Goodbye! Take care and stay healthy."
            },
            {
                "question": "'I forgive you. These things happen.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันไม่ให้อภัยคุณ",
                    "ฉันให้อภัยคุณแล้ว สิ่งเหล่านี้มันเกิดขึ้นได้",
                    "สิ่งเหล่านี้ไม่ควรเกิดขึ้น",
                ],
                "answer":   "ฉันให้อภัยคุณแล้ว สิ่งเหล่านี้มันเกิดขึ้นได้"
            },
        ]
    },

    # ============================================================
    # DAY 7: การพูดคุยเกี่ยวกับครอบครัว
    # ============================================================
    "Day 7": {
        "title": "👨‍👩‍👧 Day 7: การพูดคุยเกี่ยวกับครอบครัว",
        "phrases": [
            {
                "english":       "I come from a large family of six people.",
                "pronunciation": "ไอ คัม ฟรอม อะ ลาร์จ แฟม-มิ-ลี่ ออฟ ซิกซ์ พี-เพิล.",
                "meaning":       "ฉันมาจากครอบครัวใหญ่ที่มีสมาชิกหกคน"
            },
            {
                "english":       "My parents are both retired teachers.",
                "pronunciation": "มาย แพร์-เรนท์ส อาร์ โบธ รี-ไทร์ด ทีช-เชอร์ส.",
                "meaning":       "พ่อแม่ของฉันทั้งคู่เป็นครูที่เกษียณแล้ว"
            },
            {
                "english":       "I have one older brother and one younger sister.",
                "pronunciation": "ไอ แฮฟ วัน โอล-เดอร์ บรัธ-เธอร์ แอนด์ วัน ยัง-เกอร์ ซิส-เตอร์.",
                "meaning":       "ฉันมีพี่ชายหนึ่งคนและน้องสาวหนึ่งคน"
            },
            {
                "english":       "Are you married or still single?",
                "pronunciation": "อาร์ ยู แมร์-รีด ออร์ สทิล ซิง-เกิล?",
                "meaning":       "คุณแต่งงานแล้วหรือยังโสดอยู่คะ/ครับ?"
            },
            {
                "english":       "I'm married and we have two lovely children.",
                "pronunciation": "ไอม์ แมร์-รีด แอนด์ วี แฮฟ ทู ลัฟ-ลี่ ชิล-เดรน.",
                "meaning":       "ฉันแต่งงานแล้วและมีลูกที่น่ารักสองคน"
            },
            {
                "english":       "My son is in primary school this year.",
                "pronunciation": "มาย ซัน อีส อิน ไพร-มะ-รี่ สกูล ดิส เยียร์.",
                "meaning":       "ลูกชายของฉันอยู่ระดับประถมศึกษาปีนี้"
            },
            {
                "english":       "My wife works as a dentist at a private clinic.",
                "pronunciation": "มาย ไวฟ์ เวิร์คส์ แอส อะ เดน-ทิสท์ แอท อะ ไพร-เวท คลิน-นิค.",
                "meaning":       "ภรรยาของฉันทำงานเป็นทันตแพทย์ที่คลินิกเอกชน"
            },
            {
                "english":       "We visit my parents every Sunday without fail.",
                "pronunciation": "วี วิซ-ซิท มาย แพร์-เรนท์ส เอฟ-รี่ ซัน-เดย์ วิธ-เอาท์ เฟล.",
                "meaning":       "เราไปเยี่ยมพ่อแม่ทุกวันอาทิตย์โดยไม่ขาด"
            },
            {
                "english":       "My grandmother lives with us at home.",
                "pronunciation": "มาย แกรนด์-มัธ-เธอร์ ลีฟส์ วิธ อัส แอท โฮม.",
                "meaning":       "คุณยายของฉันอาศัยอยู่กับเราที่บ้าน"
            },
            {
                "english":       "Family support is very important for recovery.",
                "pronunciation": "แฟม-มิ-ลี่ ซัพ-พอร์ท อีส เวรี่ อิม-พอร์-แทนท์ ฟอร์ รี-คัฟ-เวอ-รี่.",
                "meaning":       "กำลังใจจากครอบครัวสำคัญมากต่อการฟื้นตัว"
            },
        ],
        "quizzes": [
            {
                "question": "'I'm married and we have two lovely children.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันโสดและมีลูกสองคน",
                    "ฉันแต่งงานแล้วและมีลูกที่น่ารักสองคน",
                    "ฉันอยากมีลูกสองคน",
                ],
                "answer":   "ฉันแต่งงานแล้วและมีลูกที่น่ารักสองคน"
            },
            {
                "question": "'Family support is very important for recovery.' แปลว่าอะไร?",
                "choices":  [
                    "ครอบครัวต้องให้กำลังใจผู้ป่วยเสมอ",
                    "กำลังใจจากครอบครัวสำคัญมากต่อการฟื้นตัว",
                    "ผู้ป่วยควรอยู่กับครอบครัวตลอดเวลา",
                ],
                "answer":   "กำลังใจจากครอบครัวสำคัญมากต่อการฟื้นตัว"
            },
            {
                "question": "ถ้าต้องการถามว่า 'คุณแต่งงานแล้วหรือยังโสดอยู่?' ควรพูดว่าอะไร?",
                "choices":  [
                    "Do you have any children?",
                    "Are you married or still single?",
                    "How long have you been married?",
                ],
                "answer":   "Are you married or still single?"
            },
        ]
    },

    # ============================================================
    # DAY 8: สิ่งที่ชอบและไม่ชอบ
    # ============================================================
    "Day 8": {
        "title": "❤️ Day 8: สิ่งที่ชอบและไม่ชอบ",
        "phrases": [
            {
                "english":       "I absolutely love working with patients.",
                "pronunciation": "ไอ แอบ-โซ-ลิวท์-ลี่ ลัฟ เวิร์ค-กิ้ง วิธ เพ-เชินท์ส.",
                "meaning":       "ฉันชอบการดูแลผู้ป่วยมากๆ เลย"
            },
            {
                "english":       "I'm not a big fan of night shifts.",
                "pronunciation": "ไอม์ น็อท อะ บิก แฟน ออฟ ไนท์ ชิฟท์ส.",
                "meaning":       "ฉันไม่ค่อยชอบการทำงานกะดึกเท่าไหร่"
            },
            {
                "english":       "What kind of food do you enjoy most?",
                "pronunciation": "วอท ไคนด์ ออฟ ฟูด ดู ยู เอน-จอย โมสท์?",
                "meaning":       "คุณชอบทานอาหารประเภทไหนมากที่สุด?"
            },
            {
                "english":       "I prefer Thai food over any other cuisine.",
                "pronunciation": "ไอ พรี-เฟอร์ ไทย ฟูด โอ-เวอร์ เอ็น-นี่ อัธ-เธอร์ คิ-วีน.",
                "meaning":       "ฉันชอบอาหารไทยมากกว่าอาหารประเภทอื่นๆ"
            },
            {
                "english":       "I can't stand loud and noisy environments.",
                "pronunciation": "ไอ แคนท์ สแตนด์ เลาด์ แอนด์ นอยซ์-ซี่ เอน-ไวร์-เรน-เมนท์ส.",
                "meaning":       "ฉันทนสภาพแวดล้อมที่เสียงดังไม่ได้เลย"
            },
            {
                "english":       "I really enjoy learning new medical terms.",
                "pronunciation": "ไอ เรียล-ลี่ เอน-จอย เลิร์น-นิ่ง นิว เมด-ดิ-คัล เทิร์มส์.",
                "meaning":       "ฉันชอบเรียนรู้คำศัพท์ทางการแพทย์ใหม่ๆ มาก"
            },
            {
                "english":       "I dislike eating fast food because it's unhealthy.",
                "pronunciation": "ไอ ดิส-ไลค์ อีท-ติ้ง ฟาสท์ ฟูด บิ-คอส อิทส์ อัน-เฮล-ธี่.",
                "meaning":       "ฉันไม่ชอบกินฟาสต์ฟู้ดเพราะไม่ดีต่อสุขภาพ"
            },
            {
                "english":       "My favourite part of the job is helping people recover.",
                "pronunciation": "มาย เฟ-เวอร์-ริท พาร์ท ออฟ เดอะ จ็อบ อีส เฮลพ์-พิ่ง พี-เพิล รี-คัฟ-เวอร์.",
                "meaning":       "ส่วนที่ฉันชอบที่สุดในงานคือการช่วยให้ผู้ป่วยฟื้นตัว"
            },
            {
                "english":       "We share the same taste in music!",
                "pronunciation": "วี แชร์ เดอะ เซม เทสท์ อิน มิว-สิค!",
                "meaning":       "เราชอบดนตรีรสนิยมเดียวกันเลย!"
            },
            {
                "english":       "I'm really into reading health and wellness books.",
                "pronunciation": "ไอม์ เรียล-ลี่ อิน-ทู รีด-ดิ้ง เฮลธ์ แอนด์ เวล-เนส บุคส์.",
                "meaning":       "ช่วงนี้ฉันหลงใหลการอ่านหนังสือด้านสุขภาพมาก"
            },
        ],
        "quizzes": [
            {
                "question": "'I'm not a big fan of night shifts.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันชอบทำงานกะดึกมาก",
                    "ฉันไม่ค่อยชอบการทำงานกะดึกเท่าไหร่",
                    "ฉันไม่เคยทำงานกะดึก",
                ],
                "answer":   "ฉันไม่ค่อยชอบการทำงานกะดึกเท่าไหร่"
            },
            {
                "question": "'I prefer Thai food over any other cuisine.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันชอบกินอาหารทุกประเภท",
                    "ฉันชอบอาหารไทยมากกว่าอาหารประเภทอื่นๆ",
                    "ฉันชอบอาหารต่างชาติมากกว่าอาหารไทย",
                ],
                "answer":   "ฉันชอบอาหารไทยมากกว่าอาหารประเภทอื่นๆ"
            },
            {
                "question": "ถ้าต้องการบอกว่า 'ฉันทนสภาพแวดล้อมที่เสียงดังไม่ได้' ควรพูดว่าอะไร?",
                "choices":  [
                    "I don't like noisy music.",
                    "I can't stand loud and noisy environments.",
                    "I prefer quiet hospitals.",
                ],
                "answer":   "I can't stand loud and noisy environments."
            },
        ]
    },

    # ============================================================
    # DAY 9: งานอดิเรกและความสนใจ
    # ============================================================
    "Day 9": {
        "title": "🎨 Day 9: งานอดิเรกและความสนใจ",
        "phrases": [
            {
                "english":       "What do you usually do in your free time?",
                "pronunciation": "วอท ดู ยู ยู-ชู-อัล-ลี่ ดู อิน ยัวร์ ฟรี ไทม์?",
                "meaning":       "ปกติคุณชอบทำอะไรในเวลาว่างคะ/ครับ?"
            },
            {
                "english":       "I enjoy painting watercolours as a hobby.",
                "pronunciation": "ไอ เอน-จอย เพนท์-ติ้ง วอ-เตอร์-คัล-เลอร์ส แอส อะ ฮ็อบ-บี้.",
                "meaning":       "ฉันชอบวาดภาพสีน้ำเป็นงานอดิเรก"
            },
            {
                "english":       "I've recently taken up yoga for relaxation.",
                "pronunciation": "ไอฟ รี-เซนท์-ลี่ เทค-เคน อัพ โยกา ฟอร์ รี-แลก-เซ-ชัน.",
                "meaning":       "เมื่อไม่นานมานี้ฉันเริ่มเล่นโยคะเพื่อผ่อนคลาย"
            },
            {
                "english":       "I love hiking in national parks on holidays.",
                "pronunciation": "ไอ ลัฟ ไฮค์-กิ้ง อิน แนช-ชะ-เนิล พาร์คส์ ออน ฮ็อล-ลิ-เดย์ส.",
                "meaning":       "ฉันชอบเดินป่าในอุทยานแห่งชาติในวันหยุด"
            },
            {
                "english":       "Reading medical journals keeps me up to date.",
                "pronunciation": "รีด-ดิ้ง เมด-ดิ-คัล เจอร์-เนิลส์ คีพส์ มี อัพ ทู เดท.",
                "meaning":       "การอ่านวารสารทางการแพทย์ทำให้ฉันอัปเดตอยู่เสมอ"
            },
            {
                "english":       "I play badminton with my friends every weekend.",
                "pronunciation": "ไอ เพลย์ แบด-มิน-ตัน วิธ มาย เฟรนด์ส เอฟ-รี่ วีค-เอนด์.",
                "meaning":       "ฉันเล่นแบดมินตันกับเพื่อนทุกสุดสัปดาห์"
            },
            {
                "english":       "I'm passionate about cooking healthy meals.",
                "pronunciation": "ไอม์ แพช-ชัน-เนท อะ-เบาท์ คุค-กิ้ง เฮล-ธี่ มีลส์.",
                "meaning":       "ฉันหลงใหลในการทำอาหารเพื่อสุขภาพมาก"
            },
            {
                "english":       "I've been learning to play the piano for two years.",
                "pronunciation": "ไอฟ บีน เลิร์น-นิ่ง ทู เพลย์ เดอะ พี-แอน-โน ฟอร์ ทู เยียร์ส.",
                "meaning":       "ฉันเรียนเปียโนมาสองปีแล้ว"
            },
            {
                "english":       "I find gardening very calming after a long shift.",
                "pronunciation": "ไอ ไฟนด์ การ์-เด้น-นิ่ง เวรี่ คาม-มิ่ง อาฟ-เตอร์ อะ ลอง ชิฟท์.",
                "meaning":       "ฉันรู้สึกสงบใจมากเมื่อทำสวนหลังจากทำงานกะยาวๆ"
            },
            {
                "english":       "Do you have any hobbies we can do together?",
                "pronunciation": "ดู ยู แฮฟ เอ็นนี่ ฮ็อบ-บีส์ วี แคน ดู ทู-เกธ-เธอร์?",
                "meaning":       "คุณมีงานอดิเรกอะไรที่เราทำด้วยกันได้บ้างคะ/ครับ?"
            },
        ],
        "quizzes": [
            {
                "question": "'I enjoy painting watercolours as a hobby.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันชอบซื้อภาพสีน้ำเป็นงานอดิเรก",
                    "ฉันชอบวาดภาพสีน้ำเป็นงานอดิเรก",
                    "ฉันสอนวาดภาพสีน้ำ",
                ],
                "answer":   "ฉันชอบวาดภาพสีน้ำเป็นงานอดิเรก"
            },
            {
                "question": "'I find gardening very calming after a long shift.' แปลว่าอะไร?",
                "choices":  [
                    "ฉันทำสวนทุกเช้าก่อนไปทำงาน",
                    "ฉันรู้สึกสงบใจมากเมื่อทำสวนหลังจากทำงานกะยาวๆ",
                    "การทำสวนทำให้ฉันเหนื่อยมาก",
                ],
                "answer":   "ฉันรู้สึกสงบใจมากเมื่อทำสวนหลังจากทำงานกะยาวๆ"
            },
            {
                "question": "ถ้าต้องการถามว่า 'ปกติคุณชอบทำอะไรในเวลาว่าง?' ควรพูดว่าอะไร?",
                "choices":  [
                    "What are you doing right now?",
                    "What do you usually do in your free time?",
                    "Do you have time to talk?",
                ],
                "answer":   "What do you usually do in your free time?"
            },
        ]
    },

    # ============================================================
    # DAY 10: งานและอาชีพ
    # ============================================================
    "Day 10": {
        "title": "💼 Day 10: งานและอาชีพ",
        "phrases": [
            {
                "english":       "What do you do for a living?",
                "pronunciation": "วอท ดู ยู ดู ฟอร์ อะ ลีฟ-วิ่ง?",
                "meaning":       "คุณทำอาชีพอะไรคะ/ครับ?"
            },
            {
                "english":       "I work as a registered nurse at a government hospital.",
                "pronunciation": "ไอ เวิร์ค แอส อะ เร็จ-จิส-เทิร์ด เนิร์ส แอท อะ กัฟ-เวิร์น-เมนท์ ฮอส-พิ-ทัล.",
                "meaning":       "ฉันทำงานเป็นพยาบาลวิชาชีพที่โรงพยาบาลรัฐ"
            },
            {
                "english":       "I have been in the healthcare field for over ten years.",
                "pronunciation": "ไอ แฮฟ บีน อิน เดอะ เฮลธ์-แคร์ ฟีลด์ ฟอร์ โอ-เวอร์ เท็น เยียร์ส.",
                "meaning":       "ฉันอยู่ในสายงานด้านสุขภาพมากกว่าสิบปีแล้ว"
            },
            {
                "english":       "My job can be challenging but very fulfilling.",
                "pronunciation": "มาย จ็อบ แคน บี ชะ-เลน-จิ่ง บัท เวรี่ ฟูล-ฟิล-ลิ่ง.",
                "meaning":       "งานของฉันอาจท้าทาย แต่ก็รู้สึกคุ้มค่ามาก"
            },
            {
                "english":       "I'm currently applying for a senior position.",
                "pronunciation": "ไอม์ เคอร์-เรนท์-ลี่ แอพ-พลาย-อิ้ง ฟอร์ อะ ซีน-เนียร์ โพ-ซิ-ชัน.",
                "meaning":       "ตอนนี้ฉันกำลังสมัครตำแหน่งอาวุโสอยู่ค่ะ/ครับ"
            },
            {
                "english":       "Teamwork and communication are key skills in healthcare.",
                "pronunciation": "ทีม-เวิร์ค แอนด์ คัม-มิว-นิ-เค-ชัน อาร์ คี สกิลส์ อิน เฮลธ์-แคร์.",
                "meaning":       "การทำงานเป็นทีมและการสื่อสารเป็นทักษะสำคัญในงานสุขภาพ"
            },
            {
                "english":       "I work from Monday to Friday on the day shift.",
                "pronunciation": "ไอ เวิร์ค ฟรอม มัน-เดย์ ทู ไฟร์-เดย์ ออน เดอะ เดย์ ชิฟท์.",
                "meaning":       "ฉันทำงานตั้งแต่วันจันทร์ถึงวันศุกร์ กะกลางวัน"
            },
            {
                "english":       "My dream is to become a specialist nurse one day.",
                "pronunciation": "มาย ดรีม อีส ทู บิ-คัม อะ สเปช-ชะ-ลิสท์ เนิร์ส วัน เดย์.",
                "meaning":       "ความฝันของฉันคืออยากเป็นพยาบาลผู้เชี่ยวชาญสักวัน"
            },
            {
                "english":       "I'm proud to serve and care for my community.",
                "pronunciation": "ไอม์ เพราด ทู เซิร์ฟ แอนด์ แคร์ ฟอร์ มาย คัม-มิว-นิ-ที่.",
                "meaning":       "ฉันภูมิใจที่ได้ดูแลและรับใช้ชุมชนของฉัน"
            },
            {
                "english":       "Continuing education is essential in the medical field.",
                "pronunciation": "คอน-ทิน-ยู-อิ้ง เอ็ด-ยู-เค-ชัน อีส เอส-เซน-ชัล อิน เดอะ เมด-ดิ-คัล ฟีลด์.",
                "meaning":       "การศึกษาต่อเนื่องเป็นสิ่งจำเป็นมากในวงการแพทย์"
            },
        ],
        "quizzes": [
            {
                "question": "'My job can be challenging but very fulfilling.' แปลว่าอะไร?",
                "choices":  [
                    "งานของฉันง่ายและน่าเบื่อมาก",
                    "งานของฉันอาจท้าทาย แต่ก็รู้สึกคุ้มค่ามาก",
                    "ฉันอยากเปลี่ยนงานใหม่",
                ],
                "answer":   "งานของฉันอาจท้าทาย แต่ก็รู้สึกคุ้มค่ามาก"
            },
            {
                "question": "'Teamwork and communication are key skills in healthcare.' แปลว่าอะไร?",
                "choices":  [
                    "การสื่อสารไม่สำคัญในงานสุขภาพ",
                    "การทำงานคนเดียวดีกว่าทำงานเป็นทีม",
                    "การทำงานเป็นทีมและการสื่อสารเป็นทักษะสำคัญในงานสุขภาพ",
                ],
                "answer":   "การทำงานเป็นทีมและการสื่อสารเป็นทักษะสำคัญในงานสุขภาพ"
            },
            {
                "question": "ถ้าต้องการบอกว่า 'ฉันภูมิใจที่ได้ดูแลชุมชนของฉัน' ควรพูดว่าอะไร?",
                "choices":  [
                    "I enjoy working with my community.",
                    "I'm proud to serve and care for my community.",
                    "My community needs more healthcare workers.",
                ],
                "answer":   "I'm proud to serve and care for my community."
            },
        ]
    },

    "Day 11": {
        "title": "การแสดงอารมณ์และความรู้สึก",
        "phrases": [
            {
                "english": "I am so happy today!",
                "pronunciation": "ไอ แอม โซ แฮปปี้ ทูเดย์",
                "meaning": "ฉันมีความสุขมากวันนี้!"
            },
            {
                "english": "She looks really sad.",
                "pronunciation": "ชี ลุคส์ เรียลลี่ แซด",
                "meaning": "เธอดูเศร้ามาก"
            },
            {
                "english": "I am angry about what happened.",
                "pronunciation": "ไอ แอม แองกรี้ อะเบาท์ วอท แฮพเพนด์",
                "meaning": "ฉันโกรธกับสิ่งที่เกิดขึ้น"
            },
            {
                "english": "He feels nervous before the exam.",
                "pronunciation": "ฮี ฟีลส์ เนอร์วัส บีฟอร์ ดิ อิกแซม",
                "meaning": "เขารู้สึกประหม่าก่อนสอบ"
            },
            {
                "english": "I am excited about the trip.",
                "pronunciation": "ไอ แอม เอ็กไซเต็ด อะเบาท์ เดอะ ทริป",
                "meaning": "ฉันตื่นเต้นกับการเดินทาง"
            },
            {
                "english": "They are surprised by the news.",
                "pronunciation": "เดย์ อาร์ เซอร์ไพรซ์ด์ บาย เดอะ นิวส์",
                "meaning": "พวกเขาประหลาดใจกับข่าวนั้น"
            },
            {
                "english": "I feel tired and need to rest.",
                "pronunciation": "ไอ ฟีล ไทร์ด แอนด์ นีด ทู เรสท์",
                "meaning": "ฉันรู้สึกเหนื่อยและต้องการพัก"
            },
            {
                "english": "She is proud of her achievement.",
                "pronunciation": "ชี อิส พราวด์ ออฟ เฮอร์ อะชีฟเมนท์",
                "meaning": "เธอภูมิใจในความสำเร็จของเธอ"
            },
            {
                "english": "I feel lonely when you are away.",
                "pronunciation": "ไอ ฟีล โลนลี่ เวน ยู อาร์ อะเวย์",
                "meaning": "ฉันรู้สึกเหงาเมื่อคุณไม่อยู่"
            },
            {
                "english": "He is worried about his health.",
                "pronunciation": "ฮี อิส วอร์ริด อะเบาท์ ฮิส เฮลธ์",
                "meaning": "เขากังวลเกี่ยวกับสุขภาพของตัวเอง"
            }
        ],
        "quizzes": [
            {
                "question": "คำว่า 'nervous' หมายความว่าอะไร?",
                "choices": ["ดีใจ", "ประหม่า / กังวล", "โกรธ", "เศร้า"],
                "answer": "ประหม่า / กังวล"
            },
            {
                "question": "ประโยคใดแสดงความรู้สึกตื่นเต้น?",
                "choices": [
                    "I feel lonely when you are away.",
                    "She looks really sad.",
                    "I am excited about the trip.",
                    "He is worried about his health."
                ],
                "answer": "I am excited about the trip."
            },
            {
                "question": "'She is proud of her achievement.' แปลว่าอะไร?",
                "choices": [
                    "เธอเหนื่อยกับการทำงาน",
                    "เธอภูมิใจในความสำเร็จของเธอ",
                    "เธอโกรธกับผลลัพธ์",
                    "เธอประหลาดใจกับรางวัล"
                ],
                "answer": "เธอภูมิใจในความสำเร็จของเธอ"
            }
        ]
    },
    "Day 12": {
        "title": "การบอกเวลา",
        "phrases": [
            {
                "english": "What time is it now?",
                "pronunciation": "วอท ไทม์ อิส อิท เนา",
                "meaning": "ตอนนี้กี่โมงแล้ว?"
            },
            {
                "english": "It is three o'clock.",
                "pronunciation": "อิท อิส ธรี โอ'คล็อค",
                "meaning": "ตอนนี้สามโมงแล้ว"
            },
            {
                "english": "The meeting starts at half past nine.",
                "pronunciation": "เดอะ มีทติ้ง สตาร์ทส์ แอท ฮาฟ พาสท์ ไนน์",
                "meaning": "การประชุมเริ่มเวลาเก้าโมงครึ่ง"
            },
            {
                "english": "I wake up at six in the morning.",
                "pronunciation": "ไอ เวค อัพ แอท ซิกซ์ อิน เดอะ มอร์นิง",
                "meaning": "ฉันตื่นนอนตีหกตอนเช้า"
            },
            {
                "english": "The train arrives at quarter to eight.",
                "pronunciation": "เดอะ เทรน อะไรฟส์ แอท ควอร์เทอร์ ทู เอท",
                "meaning": "รถไฟมาถึงเวลาแปดโมงลบสิบห้านาที"
            },
            {
                "english": "We have lunch at noon.",
                "pronunciation": "วี แฮฟ ลันช์ แอท นูน",
                "meaning": "เรากินอาหารกลางวันตอนเที่ยง"
            },
            {
                "english": "The shop closes at ten p.m.",
                "pronunciation": "เดอะ ชอป โคลเซส แอท เทน พีเอ็ม",
                "meaning": "ร้านปิดตอนสี่ทุ่ม"
            },
            {
                "english": "Can you meet me at five thirty?",
                "pronunciation": "แคน ยู มีท มี แอท ไฟฟ์ เทอร์ตี้",
                "meaning": "คุณมาพบฉันตอนห้าโมงครึ่งได้ไหม?"
            },
            {
                "english": "It is almost midnight.",
                "pronunciation": "อิท อิส ออลโมสท์ มิดไนท์",
                "meaning": "เกือบเที่ยงคืนแล้ว"
            },
            {
                "english": "The class ends in ten minutes.",
                "pronunciation": "เดอะ คลาส เอนดส์ อิน เทน มินิทส์",
                "meaning": "ชั้นเรียนจะจบในอีกสิบนาที"
            }
        ],
        "quizzes": [
            {
                "question": "'Half past nine' หมายถึงเวลาเท่าไหร่?",
                "choices": ["9:00", "9:15", "9:30", "8:30"],
                "answer": "9:30"
            },
            {
                "question": "ประโยคใดใช้ถามเวลา?",
                "choices": [
                    "It is three o'clock.",
                    "What time is it now?",
                    "We have lunch at noon.",
                    "The shop closes at ten p.m."
                ],
                "answer": "What time is it now?"
            },
            {
                "question": "'Quarter to eight' หมายถึงเวลาเท่าไหร่?",
                "choices": ["8:15", "7:45", "8:45", "7:15"],
                "answer": "7:45"
            }
        ]
    },
    "Day 13": {
        "title": "การบรรยายลักษณะผู้คน และการซื้อของ",
        "phrases": [
            {
                "english": "She has long curly hair.",
                "pronunciation": "ชี แฮส ลอง เคอร์ลี่ แฮร์",
                "meaning": "เธอมีผมยาวหยิก"
            },
            {
                "english": "He is tall and slim.",
                "pronunciation": "ฮี อิส ทอล แอนด์ สลิม",
                "meaning": "เขาสูงและผอม"
            },
            {
                "english": "How much does this shirt cost?",
                "pronunciation": "เฮา มัช ดัส ดิส เชิร์ท คอสท์",
                "meaning": "เสื้อตัวนี้ราคาเท่าไหร่?"
            },
            {
                "english": "This bag is on sale.",
                "pronunciation": "ดิส แบ็ก อิส ออน เซล",
                "meaning": "กระเป๋าใบนี้กำลังลดราคาอยู่"
            },
            {
                "english": "Do you have this in a larger size?",
                "pronunciation": "ดู ยู แฮฟ ดิส อิน อะ ลาร์เจอร์ ไซส์",
                "meaning": "คุณมีขนาดที่ใหญ่กว่านี้ไหม?"
            },
            {
                "english": "She is wearing a red dress.",
                "pronunciation": "ชี อิส แวริง อะ เรด เดรส",
                "meaning": "เธอกำลังสวมชุดสีแดงอยู่"
            },
            {
                "english": "Can I try this on?",
                "pronunciation": "แคน ไอ ทราย ดิส ออน",
                "meaning": "ฉันขอลองสวมดูได้ไหม?"
            },
            {
                "english": "He has blue eyes and a friendly smile.",
                "pronunciation": "ฮี แฮส บลู อายส์ แอนด์ อะ เฟรนด์ลี่ สไมล์",
                "meaning": "เขามีตาสีฟ้าและรอยยิ้มที่เป็นมิตร"
            },
            {
                "english": "I would like to pay by credit card.",
                "pronunciation": "ไอ วูด ไลค์ ทู เพย์ บาย เครดิต การ์ด",
                "meaning": "ฉันต้องการจ่ายด้วยบัตรเครดิต"
            },
            {
                "english": "Do you accept cash only?",
                "pronunciation": "ดู ยู แอ็คเซ็พท์ แคช โอนลี่",
                "meaning": "คุณรับแค่เงินสดเท่านั้นหรือ?"
            }
        ],
        "quizzes": [
            {
                "question": "ประโยคใดใช้เมื่อต้องการลองเสื้อผ้า?",
                "choices": [
                    "How much does this shirt cost?",
                    "Can I try this on?",
                    "This bag is on sale.",
                    "Do you accept cash only?"
                ],
                "answer": "Can I try this on?"
            },
            {
                "question": "'She has long curly hair.' แปลว่าอะไร?",
                "choices": [
                    "เธอมีผมสั้นตรง",
                    "เธอมีผมยาวหยิก",
                    "เธอมีผมสีทอง",
                    "เธอมีผมสั้นหยิก"
                ],
                "answer": "เธอมีผมยาวหยิก"
            },
            {
                "question": "หากต้องการถามราคาสินค้า ควรใช้ประโยคใด?",
                "choices": [
                    "Can I try this on?",
                    "Do you have this in a larger size?",
                    "How much does this shirt cost?",
                    "I would like to pay by credit card."
                ],
                "answer": "How much does this shirt cost?"
            }
        ]
    },
    "Day 14": {
        "title": "การบอกทาง",
        "phrases": [
            {
                "english": "Excuse me, where is the nearest bank?",
                "pronunciation": "เอ็กคิวส์ มี แวร์ อิส เดอะ เนียเรสท์ แบงค์",
                "meaning": "ขอโทษ ธนาคารที่ใกล้ที่สุดอยู่ที่ไหน?"
            },
            {
                "english": "Turn left at the traffic light.",
                "pronunciation": "เทิร์น เลฟท์ แอท เดอะ แทร็ฟฟิค ไลท์",
                "meaning": "เลี้ยวซ้ายที่สัญญาณไฟจราจร"
            },
            {
                "english": "Go straight ahead for two blocks.",
                "pronunciation": "โก สเตรท อะเฮด ฟอร์ ทู บล็อคส์",
                "meaning": "ตรงไปอีกสองช่วงตึก"
            },
            {
                "english": "Turn right after the school.",
                "pronunciation": "เทิร์น ไรท์ อาฟเทอร์ เดอะ สคูล",
                "meaning": "เลี้ยวขวาหลังจากโรงเรียน"
            },
            {
                "english": "The hotel is on the left side.",
                "pronunciation": "เดอะ โฮเทล อิส ออน เดอะ เลฟท์ ไซด์",
                "meaning": "โรงแรมอยู่ทางฝั่งซ้าย"
            },
            {
                "english": "It is about five minutes walk from here.",
                "pronunciation": "อิท อิส อะเบาท์ ไฟฟ์ มินิทส์ วอล์ค ฟรอม เฮียร์",
                "meaning": "ห่างจากที่นี่ประมาณห้านาทีเดินเท้า"
            },
            {
                "english": "Take the second street on the right.",
                "pronunciation": "เทค เดอะ เซคเคิ่นด์ สตรีท ออน เดอะ ไรท์",
                "meaning": "เข้าซอยที่สองทางขวามือ"
            },
            {
                "english": "You cannot miss it.",
                "pronunciation": "ยู แคนนอท มิส อิท",
                "meaning": "คุณจะต้องเห็นแน่นอน (ไม่มีทางพลาด)"
            },
            {
                "english": "Is it far from here?",
                "pronunciation": "อิส อิท ฟาร์ ฟรอม เฮียร์",
                "meaning": "มันอยู่ไกลจากที่นี่ไหม?"
            },
            {
                "english": "Cross the bridge and you will see the park.",
                "pronunciation": "ครอส เดอะ บริดจ์ แอนด์ ยู วิล ซี เดอะ พาร์ค",
                "meaning": "ข้ามสะพานแล้วคุณจะเห็นสวนสาธารณะ"
            }
        ],
        "quizzes": [
            {
                "question": "'Turn left at the traffic light.' แปลว่าอะไร?",
                "choices": [
                    "ตรงไปที่สัญญาณไฟ",
                    "เลี้ยวขวาที่สัญญาณไฟ",
                    "เลี้ยวซ้ายที่สัญญาณไฟจราจร",
                    "หยุดที่สัญญาณไฟ"
                ],
                "answer": "เลี้ยวซ้ายที่สัญญาณไฟจราจร"
            },
            {
                "question": "ประโยคใดใช้ถามเส้นทาง?",
                "choices": [
                    "You cannot miss it.",
                    "It is about five minutes walk.",
                    "Excuse me, where is the nearest bank?",
                    "Cross the bridge and you will see the park."
                ],
                "answer": "Excuse me, where is the nearest bank?"
            },
            {
                "question": "'Go straight ahead' หมายความว่าอะไร?",
                "choices": ["เลี้ยวซ้าย", "เลี้ยวขวา", "ถอยหลัง", "ตรงไปข้างหน้า"],
                "answer": "ตรงไปข้างหน้า"
            }
        ]
    },
    "Day 15": {
        "title": "การไปพบแพทย์",
        "phrases": [
            {
                "english": "I would like to make an appointment with the doctor.",
                "pronunciation": "ไอ วูด ไลค์ ทู เมค แอน อะพอยท์เมนท์ วิธ เดอะ ด็อคเตอร์",
                "meaning": "ฉันต้องการนัดพบแพทย์"
            },
            {
                "english": "I have a headache and a fever.",
                "pronunciation": "ไอ แฮฟ อะ เฮดเอค แอนด์ อะ ฟีเวอร์",
                "meaning": "ฉันมีอาการปวดหัวและเป็นไข้"
            },
            {
                "english": "My throat is very sore.",
                "pronunciation": "มาย โธรท อิส เวรี่ ซอร์",
                "meaning": "ลำคอของฉันเจ็บมาก"
            },
            {
                "english": "How long have you had this pain?",
                "pronunciation": "เฮา ลอง แฮฟ ยู แฮด ดิส เพน",
                "meaning": "คุณมีอาการปวดนี้มานานแค่ไหนแล้ว?"
            },
            {
                "english": "I have been feeling sick for three days.",
                "pronunciation": "ไอ แฮฟ บีน ฟีลิง ซิค ฟอร์ ธรี เดย์ส",
                "meaning": "ฉันรู้สึกไม่สบายมาสามวันแล้ว"
            },
            {
                "english": "Are you allergic to any medicine?",
                "pronunciation": "อาร์ ยู อะเลอร์จิค ทู เอนนี่ เมดิซิน",
                "meaning": "คุณแพ้ยาตัวใดหรือไม่?"
            },
            {
                "english": "Please take this medicine twice a day.",
                "pronunciation": "พลีส เทค ดิส เมดิซิน ทไวส์ อะ เดย์",
                "meaning": "กรุณาทานยานี้วันละสองครั้ง"
            },
            {
                "english": "I have a pain in my stomach.",
                "pronunciation": "ไอ แฮฟ อะ เพน อิน มาย สตัมมัค",
                "meaning": "ฉันมีอาการปวดท้อง"
            },
            {
                "english": "You need to get some rest.",
                "pronunciation": "ยู นีด ทู เก็ท ซัม เรสท์",
                "meaning": "คุณต้องพักผ่อน"
            },
            {
                "english": "I will write you a prescription.",
                "pronunciation": "ไอ วิล ไรท์ ยู อะ พรีสคริปชัน",
                "meaning": "ฉันจะเขียนใบสั่งยาให้คุณ"
            }
        ],
        "quizzes": [
            {
                "question": "ประโยคใดใช้บอกอาการเจ็บป่วย?",
                "choices": [
                    "I will write you a prescription.",
                    "Are you allergic to any medicine?",
                    "I have a headache and a fever.",
                    "You need to get some rest."
                ],
                "answer": "I have a headache and a fever."
            },
            {
                "question": "'Are you allergic to any medicine?' แปลว่าอะไร?",
                "choices": [
                    "คุณทานยาวันละกี่ครั้ง?",
                    "คุณแพ้ยาตัวใดหรือไม่?",
                    "คุณซื้อยาที่ไหน?",
                    "คุณมีใบสั่งยาไหม?"
                ],
                "answer": "คุณแพ้ยาตัวใดหรือไม่?"
            },
            {
                "question": "'Prescription' หมายถึงอะไร?",
                "choices": ["ใบเสร็จ", "ใบนัด", "ใบสั่งยา", "ผลการตรวจ"],
                "answer": "ใบสั่งยา"
            }
        ]
    },
    "Day 16": {
        "title": "การพูดคุยเกี่ยวกับทักษะและความสามารถ",
        "phrases": [
            {
                "english": "I can speak three languages.",
                "pronunciation": "ไอ แคน สปีค ธรี แลงกวิดเจส",
                "meaning": "ฉันพูดได้สามภาษา"
            },
            {
                "english": "She is very good at cooking.",
                "pronunciation": "ชี อิส เวรี่ กุด แอท คุกกิ้ง",
                "meaning": "เธอทำอาหารเก่งมาก"
            },
            {
                "english": "He cannot swim at all.",
                "pronunciation": "ฮี แคนนอท สวิม แอท ออล",
                "meaning": "เขาว่ายน้ำไม่เป็นเลย"
            },
            {
                "english": "I know how to play the guitar.",
                "pronunciation": "ไอ โน ฮาว ทู เพลย์ เดอะ กีตาร์",
                "meaning": "ฉันรู้วิธีเล่นกีตาร์"
            },
            {
                "english": "She is skilled at drawing.",
                "pronunciation": "ชี อิส สกิลด์ แอท ดรอว์อิง",
                "meaning": "เธอมีทักษะในการวาดภาพ"
            },
            {
                "english": "I am still learning how to drive.",
                "pronunciation": "ไอ แอม สติล เลิร์นนิง ฮาว ทู ไดรฟ์",
                "meaning": "ฉันยังเรียนขับรถอยู่"
            },
            {
                "english": "He has excellent leadership skills.",
                "pronunciation": "ฮี แฮส เอ็กเซลเลนท์ ลีดเดอร์ชิพ สกิลส์",
                "meaning": "เขามีทักษะความเป็นผู้นำที่ยอดเยี่ยม"
            },
            {
                "english": "I am not very good at math.",
                "pronunciation": "ไอ แอม น็อท เวรี่ กุด แอท แมธ",
                "meaning": "ฉันไม่ค่อยเก่งคณิตศาสตร์"
            },
            {
                "english": "Can you teach me how to use this software?",
                "pronunciation": "แคน ยู ทีช มี ฮาว ทู ยูส ดิส ซอฟท์แวร์",
                "meaning": "คุณสอนวิธีใช้ซอฟต์แวร์นี้ให้ฉันได้ไหม?"
            },
            {
                "english": "She can type very fast.",
                "pronunciation": "ชี แคน ไทพ์ เวรี่ ฟาสท์",
                "meaning": "เธอพิมพ์ดีดได้เร็วมาก"
            }
        ],
        "quizzes": [
            {
                "question": "ประโยคใดแสดงว่าทำสิ่งใดสิ่งหนึ่งไม่ได้เลย?",
                "choices": [
                    "I can speak three languages.",
                    "She can type very fast.",
                    "He cannot swim at all.",
                    "I know how to play the guitar."
                ],
                "answer": "He cannot swim at all."
            },
            {
                "question": "'She is skilled at drawing.' แปลว่าอะไร?",
                "choices": [
                    "เธอกำลังเรียนวาดภาพ",
                    "เธอมีทักษะในการวาดภาพ",
                    "เธอชอบดูภาพวาด",
                    "เธอซื้ออุปกรณ์วาดภาพ"
                ],
                "answer": "เธอมีทักษะในการวาดภาพ"
            },
            {
                "question": "หากต้องการบอกว่าตนเองไม่เก่งในสิ่งใด ควรใช้โครงสร้างใด?",
                "choices": [
                    "I can ... very well.",
                    "I am not very good at ...",
                    "I know how to ...",
                    "I am skilled at ..."
                ],
                "answer": "I am not very good at ..."
            }
        ]
    },
    "Day 17": {
        "title": "การพูดคุยเกี่ยวกับวันหยุด",
        "phrases": [
            {
                "english": "Where did you go for your holiday?",
                "pronunciation": "แวร์ ดิด ยู โก ฟอร์ ยัวร์ ฮอลิเดย์",
                "meaning": "คุณไปไหนในช่วงวันหยุด?"
            },
            {
                "english": "I spent my vacation in Japan.",
                "pronunciation": "ไอ สเพนท์ มาย เวเคชัน อิน เจแปน",
                "meaning": "ฉันใช้เวลาพักร้อนอยู่ที่ญี่ปุ่น"
            },
            {
                "english": "We stayed at a beachside resort.",
                "pronunciation": "วี สเตย์ด แอท อะ บีชไซด์ รีสอร์ท",
                "meaning": "เราพักอยู่ที่รีสอร์ทริมทะเล"
            },
            {
                "english": "The trip was absolutely wonderful.",
                "pronunciation": "เดอะ ทริป วอส แอบโซลูทลี่ วันเดอร์ฟูล",
                "meaning": "การเดินทางครั้งนั้นวิเศษมากจริงๆ"
            },
            {
                "english": "I went sightseeing around the old town.",
                "pronunciation": "ไอ เวนท์ ไซท์ซีอิง อะราวด์ เดอะ โอลด์ ทาวน์",
                "meaning": "ฉันไปเที่ยวชมเมืองเก่า"
            },
            {
                "english": "We tried a lot of local food.",
                "pronunciation": "วี ทราย อะ ล็อท ออฟ โลเคิล ฟูด",
                "meaning": "พวกเราลองกินอาหารท้องถิ่นหลายอย่าง"
            },
            {
                "english": "I took hundreds of photos there.",
                "pronunciation": "ไอ ทุค ฮันเดรดส์ ออฟ โฟโตส แดร์",
                "meaning": "ฉันถ่ายรูปไปหลายร้อยรูปที่นั่น"
            },
            {
                "english": "We had so much fun at the theme park.",
                "pronunciation": "วี แฮด โซ มัช ฟัน แอท เดอะ ธีม พาร์ค",
                "meaning": "พวกเราสนุกมากที่สวนสนุก"
            },
            {
                "english": "I would love to go back there someday.",
                "pronunciation": "ไอ วูด เลิฟ ทู โก แบ็ค แดร์ ซัมเดย์",
                "meaning": "ฉันอยากกลับไปที่นั่นอีกสักครั้ง"
            },
            {
                "english": "My favourite part was the sunset cruise.",
                "pronunciation": "มาย เฟเวอริท พาร์ท วอส เดอะ ซันเซ็ท ครูส",
                "meaning": "ส่วนที่ฉันชอบที่สุดคือทัวร์ล่องเรือชมพระอาทิตย์ตก"
            }
        ],
        "quizzes": [
            {
                "question": "'I spent my vacation in Japan.' แปลว่าอะไร?",
                "choices": [
                    "ฉันอยากไปเที่ยวญี่ปุ่น",
                    "ฉันใช้เวลาพักร้อนอยู่ที่ญี่ปุ่น",
                    "ฉันเพิ่งกลับจากญี่ปุ่น",
                    "ฉันวางแผนจะไปญี่ปุ่น"
                ],
                "answer": "ฉันใช้เวลาพักร้อนอยู่ที่ญี่ปุ่น"
            },
            {
                "question": "ประโยคใดแสดงถึงการชื่นชอบกิจกรรมในวันหยุด?",
                "choices": [
                    "Where did you go for your holiday?",
                    "We stayed at a beachside resort.",
                    "My favourite part was the sunset cruise.",
                    "I took hundreds of photos there."
                ],
                "answer": "My favourite part was the sunset cruise."
            },
            {
                "question": "'Sightseeing' หมายถึงอะไร?",
                "choices": ["การซื้อของ", "การท่องเที่ยวชมสถานที่", "การถ่ายรูป", "การลองอาหาร"],
                "answer": "การท่องเที่ยวชมสถานที่"
            }
        ]
    },
    "Day 18": {
        "title": "การพูดคุยเกี่ยวกับสภาพอากาศ",
        "phrases": [
            {
                "english": "What is the weather like today?",
                "pronunciation": "วอท อิส เดอะ เวเธอร์ ไลค์ ทูเดย์",
                "meaning": "วันนี้อากาศเป็นยังไงบ้าง?"
            },
            {
                "english": "It is very hot and humid.",
                "pronunciation": "อิท อิส เวรี่ ฮ็อท แอนด์ ฮิวมิด",
                "meaning": "อากาศร้อนและชื้นมาก"
            },
            {
                "english": "It is raining heavily outside.",
                "pronunciation": "อิท อิส เรนนิง เฮวิลี่ เอาท์ไซด์",
                "meaning": "ข้างนอกฝนตกหนักมาก"
            },
            {
                "english": "Take an umbrella just in case.",
                "pronunciation": "เทค แอน อัมเบรลลา จัสท์ อิน เคส",
                "meaning": "พกร่มไว้เผื่อไว้ด้วยนะ"
            },
            {
                "english": "The forecast says it will snow tomorrow.",
                "pronunciation": "เดอะ ฟอร์คาสท์ เซส อิท วิล สโน ทูมอร์โรว์",
                "meaning": "พยากรณ์อากาศบอกว่าพรุ่งนี้จะมีหิมะตก"
            },
            {
                "english": "It is a beautiful sunny day.",
                "pronunciation": "อิท อิส อะ บิวทิฟูล ซันนี่ เดย์",
                "meaning": "วันนี้เป็นวันที่อากาศแดดดีสวยงาม"
            },
            {
                "english": "The temperature is below zero.",
                "pronunciation": "เดอะ เทมเพอราเจอร์ อิส บีโลว์ ซีโร",
                "meaning": "อุณหภูมิอยู่ต่ำกว่าศูนย์องศา"
            },
            {
                "english": "It looks like a storm is coming.",
                "pronunciation": "อิท ลุคส์ ไลค์ อะ สตอร์ม อิส คัมมิง",
                "meaning": "ดูเหมือนพายุกำลังมา"
            },
            {
                "english": "I love this cool, breezy weather.",
                "pronunciation": "ไอ เลิฟ ดิส คูล บรีซี่ เวเธอร์",
                "meaning": "ฉันชอบอากาศเย็นและมีลมพัดแบบนี้"
            },
            {
                "english": "The weather here is unpredictable.",
                "pronunciation": "เดอะ เวเธอร์ เฮียร์ อิส อันพรีดิคทะเบิล",
                "meaning": "อากาศที่นี่คาดเดาไม่ได้"
            }
        ],
        "quizzes": [
            {
                "question": "ประโยคใดใช้ถามสภาพอากาศ?",
                "choices": [
                    "It is very hot and humid.",
                    "What is the weather like today?",
                    "Take an umbrella just in case.",
                    "The temperature is below zero."
                ],
                "answer": "What is the weather like today?"
            },
            {
                "question": "'Humid' หมายความว่าอะไร?",
                "choices": ["หนาว", "ชื้น", "แห้ง", "มีลม"],
                "answer": "ชื้น"
            },
            {
                "question": "'The weather is unpredictable.' แปลว่าอะไร?",
                "choices": [
                    "อากาศดีมาก",
                    "อากาศเปลี่ยนแปลงบ่อย",
                    "อากาศคาดเดาไม่ได้",
                    "อากาศหนาวมาก"
                ],
                "answer": "อากาศคาดเดาไม่ได้"
            }
        ]
    },
    "Day 19": {
        "title": "การพูดคุยกับเหตุการณ์ในอดีต",
        "phrases": [
            {
                "english": "I visited Paris last summer.",
                "pronunciation": "ไอ วิซิทิด แพริส ลาสท์ ซัมเมอร์",
                "meaning": "ฉันไปเยือนปารีสเมื่อฤดูร้อนที่แล้ว"
            },
            {
                "english": "She finished the project on time.",
                "pronunciation": "ชี ฟินิชท์ เดอะ โปรเจคท์ ออน ไทม์",
                "meaning": "เธอทำโปรเจกต์เสร็จทันเวลา"
            },
            {
                "english": "We met each other in university.",
                "pronunciation": "วี เมท อีช อาเธอร์ อิน ยูนิเวอร์ซิตี้",
                "meaning": "เราพบกันที่มหาวิทยาลัย"
            },
            {
                "english": "I did not sleep well last night.",
                "pronunciation": "ไอ ดิด น็อท สลีพ เวล ลาสท์ ไนท์",
                "meaning": "เมื่อคืนฉันนอนไม่หลับ"
            },
            {
                "english": "He lost his wallet yesterday.",
                "pronunciation": "ฮี ลอสท์ ฮิส วอลเล็ท เยสเตอร์เดย์",
                "meaning": "เขาทำกระเป๋าสตางค์หายเมื่อวานนี้"
            },
            {
                "english": "They moved to a new city two years ago.",
                "pronunciation": "เดย์ มูฟด์ ทู อะ นิว ซิตี้ ทู เยียส อะโก",
                "meaning": "พวกเขาย้ายไปเมืองใหม่เมื่อสองปีก่อน"
            },
            {
                "english": "I used to work as a teacher.",
                "pronunciation": "ไอ ยูสด์ ทู เวิร์ค แอส อะ ทีเชอร์",
                "meaning": "ฉันเคยทำงานเป็นครูมาก่อน"
            },
            {
                "english": "She called me early this morning.",
                "pronunciation": "ชี คอลด์ มี เอิร์ลลี่ ดิส มอร์นิง",
                "meaning": "เธอโทรหาฉันแต่เช้าวันนี้"
            },
            {
                "english": "What did you do last weekend?",
                "pronunciation": "วอท ดิด ยู ดู ลาสท์ วีคเอนด์",
                "meaning": "คุณทำอะไรเมื่อสุดสัปดาห์ที่แล้ว?"
            },
            {
                "english": "We had a great time at the party.",
                "pronunciation": "วี แฮด อะ เกรท ไทม์ แอท เดอะ พาร์ตี้",
                "meaning": "พวกเราสนุกมากในงานปาร์ตี้"
            }
        ],
        "quizzes": [
            {
                "question": "'I used to work as a teacher.' แปลว่าอะไร?",
                "choices": [
                    "ฉันกำลังทำงานเป็นครู",
                    "ฉันอยากเป็นครู",
                    "ฉันเคยทำงานเป็นครูมาก่อน",
                    "ฉันจะไปเป็นครู"
                ],
                "answer": "ฉันเคยทำงานเป็นครูมาก่อน"
            },
            {
                "question": "ประโยคใดอยู่ในรูป Past Tense (เหตุการณ์ที่ผ่านมาแล้ว)?",
                "choices": [
                    "I will visit Paris next year.",
                    "She is finishing the project.",
                    "He lost his wallet yesterday.",
                    "They are moving to a new city."
                ],
                "answer": "He lost his wallet yesterday."
            },
            {
                "question": "ประโยคใดใช้ถามเกี่ยวกับกิจกรรมในอดีต?",
                "choices": [
                    "What will you do next weekend?",
                    "What did you do last weekend?",
                    "What are you doing this weekend?",
                    "What do you do every weekend?"
                ],
                "answer": "What did you do last weekend?"
            }
        ]
    },
    "Day 20": {
        "title": "การพูดคุยในอนาคตและการคาดการณ์",
        "phrases": [
            {
                "english": "I am going to travel abroad next year.",
                "pronunciation": "ไอ แอม โกอิ้ง ทู แทรเวล อะบรอด เน็กซท์ เยียร์",
                "meaning": "ฉันจะเดินทางไปต่างประเทศปีหน้า"
            },
            {
                "english": "She will probably get the job.",
                "pronunciation": "ชี วิล พรอบบะบลี่ เก็ท เดอะ จ็อบ",
                "meaning": "เธอน่าจะได้งานนั้น"
            },
            {
                "english": "I think it is going to rain later.",
                "pronunciation": "ไอ ธิงค์ อิท อิส โกอิ้ง ทู เรน เลเทอร์",
                "meaning": "ฉันคิดว่าต่อมาจะมีฝนตก"
            },
            {
                "english": "We are planning to open a new branch.",
                "pronunciation": "วี อาร์ แพลนนิง ทู โอเพน อะ นิว บรานช์",
                "meaning": "เรากำลังวางแผนจะเปิดสาขาใหม่"
            },
            {
                "english": "He might arrive late tonight.",
                "pronunciation": "ฮี ไมท์ อะไรฟ์ เลท ทูไนท์",
                "meaning": "เขาอาจมาถึงช้าคืนนี้"
            },
            {
                "english": "I predict prices will rise next month.",
                "pronunciation": "ไอ พรีดิคท์ พราย ซิส วิล ไรส์ เน็กซท์ มันธ์",
                "meaning": "ฉันคาดว่าราคาจะสูงขึ้นเดือนหน้า"
            },
            {
                "english": "They are likely to win the competition.",
                "pronunciation": "เดย์ อาร์ ไลค์ลี่ ทู วิน เดอะ คอมพีทิชัน",
                "meaning": "พวกเขามีแนวโน้มที่จะชนะการแข่งขัน"
            },
            {
                "english": "I will call you as soon as I arrive.",
                "pronunciation": "ไอ วิล คอล ยู แอส ซูน แอส ไอ อะไรฟ์",
                "meaning": "ฉันจะโทรหาคุณทันทีที่มาถึง"
            },
            {
                "english": "Technology will change our lives dramatically.",
                "pronunciation": "เทคโนโลจี่ วิล เชนจ์ เอาร์ ไลฟส์ ดราแมทิคัลลี่",
                "meaning": "เทคโนโลยีจะเปลี่ยนแปลงชีวิตของเราอย่างมาก"
            },
            {
                "english": "What do you think will happen next?",
                "pronunciation": "วอท ดู ยู ธิงค์ วิล แฮพเพน เน็กซท์",
                "meaning": "คุณคิดว่าจะเกิดอะไรขึ้นต่อไป?"
            }
        ],
        "quizzes": [
            {
                "question": "ประโยคใดแสดงการคาดการณ์ในอนาคต?",
                "choices": [
                    "I visited Paris last summer.",
                    "She finished the project on time.",
                    "I predict prices will rise next month.",
                    "We met each other in university."
                ],
                "answer": "I predict prices will rise next month."
            },
            {
                "question": "'He might arrive late tonight.' หมายความว่าอะไร?",
                "choices": [
                    "เขามาถึงช้าคืนนี้",
                    "เขาอาจมาถึงช้าคืนนี้",
                    "เขาจะมาถึงเร็วคืนนี้",
                    "เขาไม่มาคืนนี้"
                ],
                "answer": "เขาอาจมาถึงช้าคืนนี้"
            },
            {
                "question": "คำใดแสดงถึงความเป็นไปได้ (possibility) ในอนาคต?",
                "choices": ["did", "was", "might", "had"],
                "answer": "might"
            }
        ]
    },

}  # จบ lessons_data